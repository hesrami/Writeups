...
user.txt
```
In the users home directory, there's a hidden remember_password file containing a possible password who doesnt love those lol, however you can just generate a new rsa key on your box `ssh-keygen -t rsa` and copy the public key and append that to the authorized_keys file of the user
```shell
...
we can establish a stable shell with ssh and continue to pwn the box - in the /opt/ directory there is a `service.ps1` powershell script that belongs to the root user and is writable by us
```shell
